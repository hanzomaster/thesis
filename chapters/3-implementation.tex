...
Đặc biệt trong giờ cao điểm khi các nhà hàng, quán ăn đón một lượng lớn khách hàng đến.
Từ đó khóa luận này đưa ra phương án triển khai hệ thống với mô hình vi dịch vụ (microservice) với K8s giúp tối ưu hóa chi phí vận hành và đảm bảo tính sẵn sàng cao của hệ thống.
...
Các ứng dụng đóng gói ở đây có thể hiểu như là từng dịch vụ riêng lẻ đảm nhiệm cho từng chức năng trong hệ thống trong mô hình vi dịch vụ (microservice).
Đơn vị triển khai cơ bản trong K8s được gọi là Pod.
Mỗi Pod có thể bao gồm một hoặc nhiều container

Tuy nhiên trong quá trình phát triển hệ thống, mã nguồn của chương trình liên tục trải qua thay đổi 
\subsection{RabbitMQ}
\subsection{MongoDB}
\subsection{Kong}
